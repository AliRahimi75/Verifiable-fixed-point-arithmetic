...
def MLE_renov_time_version(C, r, order, prime_number, factors, base_group): 
    N = len(C) 
    Gen = generators(2 * N + 1, prime_number, factors, base_group)     
    t_old = time.time()
    v = MLE_vector_renov(r, order)   
    t = innerproduct(C, v, order)     
    P = commit(Gen, C + v + [t], prime_number)  
    t_new = time.time() 
    t1 = t_new - t_old 
    ...
    return [t, t_total[0] + t1, t_total[1]]  
...
#    print("Verifier updates P and Generators.") 
    ...
    if n == 1: 
        ...
        try:
            aux1 = [prover_time, verifier_time]
