...
def senary_single_var_eval(g0, g1, g2, g3, t, p): 
    aux1 = (-g0 + 3*g1 - 3*g2 + g3) % p 
    aux2 = (6*g0 - 15*g1 + 12*g2 -3*g3) % p 
    aux3 = (-11*g0 + 18*g1 - 9*g2 + 2*g3) % p
    aux4 = t**3 * aux1 % p 
    return (aux4 + t**2 * aux2 + t * aux3 + 6*g0) % p
...
def round_vector(a, e, precision, bias, p): 
    ...
    for i in range(len(a)):
        b = [] 
        for j in range(len(a[0])):
            b.append(((((a[i][j] - e[i][j]) >> precision) + bias) % p) - bias)   
        c.append(b) 
    return c 


def abs(a):
    return [max(-a[i], a[i]) for i in range(len(a))]

def relu(a, b):
    return [int((a[i] + b[i]) / 2) for i in range(len(a))]

def activcation_quadratic(a, bias, p):
    b = []
    for i in range(len(a)):
        b.append(((a[i]**2 + bias) % p) - bias)
    return [b] 
...
def list_reshape(a, vertical_length, horizontal_length): 
    b = [] 
    for i in range(vertical_length): 
        b.append(a[i * horizontal_length : (i + 1) * horizontal_length])          
    return b
...
def prime_order(item):
    match item:
        ...
        case 21:
            prime_number = 539434157885944688270439914390440790598746479791103 # 169 bits 
            order = 572647725993571855913418168142718461357480339481 # 159 bits 
            factors = [2, 3, 157, 572647725993571855913418168142718461357480339481]  # factors of (prime_number - 1) / order
            base_group = 2 * 3 * 157 # base_group * order = prime_number - 1  
        case 22:
            prime_number = 71648260520221076780331562266959906023397 # 136 bits 
            order = 78218625022075411332239696797991163781 # 126 bits 
            factors = [2, 229, 78218625022075411332239696797991163781]  # factors of (prime_number - 1) / order
            base_group = 2**2 * 229 # base_group * order = prime_number - 1  
        case 23:
            prime_number = 391464207346485528265267 # 79 bits 
            order = 21748011519249196014737 # 75 bits 
            factors = [2, 3, 21748011519249196014737]  # factors of (prime_number - 1) / order
            base_group = 2 * 3**2 # base_group * order = prime_number - 1  
        case 24:
            prime_number = 258704108172589744351091181913 # 98 bits 
            order = 10779337840524572681295465913 # 94 bits 
            factors = [2, 3, 10779337840524572681295465913]  # factors of (prime_number - 1) / order
            base_group = 2**3 * 3 # base_group * order = prime_number - 1  
        case 25:
            prime_number = 2584380563628875127615677688803227 # 98 bits 
            order = 1322127972011029700487958571953 # 94 bits 
            factors = [2, 13, 17, 1322127972011029700487958571953]  # factors of (prime_number - 1) / order
            base_group = 2 * 13 * 17 # base_group * order = prime_number - 1  
        case 26:
            prime_number = 2631863545342243332035267579407711 # 112 bits 
            order = 5160516755573026141245622704721 # 103 bits 
            factors = [2, 3, 5, 17, 5160516755573026141245622704721]  # factors of (prime_number - 1) / order
            base_group = 2 * 3 * 5 * 17 # base_group * order = prime_number - 1  
        case 27:
            prime_number = 73027040754421126057726333 # 86 bits 
            order = 2028528909845031279381287 # 81 bits 
            factors = [2, 3, 2028528909845031279381287]  # factors of (prime_number - 1) / order
            base_group = 2**2 * 3**2 # base_group * order = prime_number - 1  
        case 28:
            prime_number = 3819746465318503235947 # 72 bits 
            order = 636624410886417205991 # 70 bits 
            factors = [2, 3, 636624410886417205991]  # factors of (prime_number - 1) / order
            base_group = 2 * 3 # base_group * order = prime_number - 1  
        case 29:
            prime_number = 573473318869789393487 # 69 bits 
            order = 3629577967530312617 # 62 bits 
            factors = [2, 79, 3629577967530312617]  # factors of (prime_number - 1) / order
            base_group = 2 * 79 # base_group * order = prime_number - 1  
        case 30:
            prime_number = 93552428506822318552525985816988959 # 117 bits 
            order = 46776214253411159276262992908494479 # 116 bits 
            factors = [2, 46776214253411159276262992908494479]  # factors of (prime_number - 1) / order
            base_group = 2 # base_group * order = prime_number - 1           
        case 31:
            prime_number = 243891602317407750737298110598099167326573 # 138 bits 
            order = 20324300193117312561441509216508263943881 # 134 bits 
            factors = [2, 3, 20324300193117312561441509216508263943881]  # factors of (prime_number - 1) / order
            base_group = 2**2 * 3 # base_group * order = prime_number - 1                
        case 32:
            prime_number = 70093946357765696504942095650620128562080663 # 146 bits 
            order = 11682324392960949417490349275103354760346777 # 144 bits 
            factors = [2, 3, 11682324392960949417490349275103354760346777]  # factors of (prime_number - 1) / order
            base_group = 2 * 3 # base_group * order = prime_number - 1       
        case 33:
            prime_number = 768329191341767406164317358829638051506076111975276041 # 180 bits 
            order = 6402743261181395051369311323580317095883967599793967 # 173 bits 
            factors = [2, 3, 5, 6402743261181395051369311323580317095883967599793967]  # factors of (prime_number - 1) / order
            base_group = 2**3 * 3 * 5 # base_group * order = prime_number - 1  
        case 34:
            prime_number = 6599 # 13 bits 
            order = 3299 # 12 bits 
            factors = [2, 3299]  # factors of (prime_number - 1) / order
            base_group = 2 # base_group * order = prime_number - 1  
