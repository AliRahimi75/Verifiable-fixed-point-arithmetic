...
for i in range(8):
    ...
    B = np.ceil(B * x) 
    [p_main_load, p_time, v_time, m_time] = MatMul.Thaler_method(A,B) 
    # print(p_time, v_time)
    ...
    Vector_m_time = np.concatenate((Vector_m_time, [m_time])) 
    """ 
    plt.plot(n, p_time, 'o')
...
# plt.plot(Vector_n, Vector_m_time)
